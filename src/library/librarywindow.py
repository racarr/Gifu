...
import gifu.sgf.database
import gifu.review.reviewwindow

class LibraryWindow(gtk.Window):
    
    def __play_clicked(self, pb):
        pass
        db = gifu.sgf.database.get_default()
        game = self.browser.get_selected_game()
        rw = gifu.review.reviewwindow.ReviewWindow()
    ...
    def __open_response(self, dialog, response):
        if response == gtk.RESPONSE_ACCEPT:
            ...
            print folder + " folder"
            d = gifu.sgf.database.get_default()
            d.add_folder(folder)
